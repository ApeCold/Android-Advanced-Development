...
extern "C"
JNIEXPORT void JNICALL
Java_com_netease_player_NEPlayer_releaseNative(JNIEnv *env, jobject thiz) {
    pthread_mutex_lock(&mutex);
    if (window) {
        //把老的释放
        ANativeWindow_release(window);
...
extern "C"
JNIEXPORT void JNICALL
Java_com_netease_player_NEPlayer_stopNative(JNIEnv *env, jobject thiz) {
    if (ffmpeg) {
        ffmpeg->stop();
    }
}
...
Java_com_netease_player_NEPlayer_getDurationNative(JNIEnv *env, jobject thiz) {
    if (ffmpeg) {
...
extern "C"
JNIEXPORT void JNICALL
Java_com_netease_player_NEPlayer_seekToNative(JNIEnv *env, jobject thiz, jint playProgress) {
    if (ffmpeg) {
