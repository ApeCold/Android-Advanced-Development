...
void VideoChannel::initVideoEncoder(int width, int height, int fps, int bitrate) {
    ...
    //TODO 先对 videoEncoder 和 pic_in 判断
    if (videoEncoder) {
        x264_encoder_close(videoEncoder);
        videoEncoder = 0;
    }
...
void VideoChannel::encodeData(int8_t *data) {
    ...
    //进行编码
    int ret = x264_encoder_encode(videoEncoder, &nal, &pi_nal, pic_in, &pic_out);//TODO
    if (ret < 0) {
        LOGE("x264编码失败");
        pthread_mutex_unlock(&mutex);
        return;
    }
    //sps pps ：告诉我们如何解码图像数据
    int sps_len, pps_len;
    uint8_t sps[100];
    uint8_t pps[100];
    pic_in->i_pts += 1;
...
//    packet->m_body[i++] = 0xE1;//TODO 上节课的关键bug点
