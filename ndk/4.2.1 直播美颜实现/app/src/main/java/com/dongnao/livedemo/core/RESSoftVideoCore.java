...
import com.dongnao.livedemo.client.CallbackDelivery;
...
import com.dongnao.livedemo.model.RESVideoBuff;
import com.dongnao.livedemo.render.GLESRender;
import com.dongnao.livedemo.render.IRender;
import com.dongnao.livedemo.render.NativeRender;
...
public class RESSoftVideoCore implements RESVideoCore {
    ...
    public RESSoftVideoCore(RTMPCoreParameters parameters) {
    ...
    public boolean startStreaming(RESFlvDataCollecter flvDataCollecter) {
        synchronized (syncOp) {
            try {
                ...
                LogTools.trace("RTMPVideoClient.start()failed", e);
                return false;
            }
            return true;
        }
    ...
    @Override
    public void reSetVideoSize(RTMPCoreParameters newParameters) {
    ...
    public void startPreview(SurfaceTexture surfaceTexture, int visualWidth, int visualHeight) {
        synchronized (syncPreview) {
            ...
            switch (resCoreParameters.renderingMode) {
                case RTMPCoreParameters.RENDERING_MODE_NATIVE_WINDOW:
                    previewRender = new NativeRender();
                    break;
                case RTMPCoreParameters.RENDERING_MODE_OPENGLES:
