...
import com.dongnao.livedemo.model.MediaCodecGLWapper;
import com.dongnao.livedemo.model.OffScreenGLWapper;
import com.dongnao.livedemo.model.RTMPCoreParameters;
import com.dongnao.livedemo.model.ScreenGLWapper;
import com.dongnao.livedemo.tools.GLESTools;
...
//            0.0f, 1.0f};
    ...
    public static FloatBuffer getCamera2DTextureVerticesBuffer(final int directionFlag, final float cropRatio) {
        ...
        switch (directionFlag & 0xF0) {
            case RTMPCoreParameters.FLAG_DIRECTION_ROATATION_90:
                buffer = Cam2dTextureVertices_90.clone();
                break;
            case RTMPCoreParameters.FLAG_DIRECTION_ROATATION_180:
                buffer = Cam2dTextureVertices_180.clone();
                break;
            case RTMPCoreParameters.FLAG_DIRECTION_ROATATION_270:
                buffer = Cam2dTextureVertices_270.clone();
                break;
            default:
                buffer = Cam2dTextureVertices.clone();
        }
        if ((directionFlag & 0xF0) == RTMPCoreParameters.FLAG_DIRECTION_ROATATION_0 || (directionFlag & 0xF0) == RTMPCoreParameters.FLAG_DIRECTION_ROATATION_180) {
            if (cropRatio > 0) {
        ...
        if ((directionFlag & RTMPCoreParameters.FLAG_DIRECTION_FLIP_HORIZONTAL) != 0) {
            buffer[0] = flip(buffer[0]);
            buffer[2] = flip(buffer[2]);
            buffer[4] = flip(buffer[4]);
            buffer[6] = flip(buffer[6]);
        }
        if ((directionFlag & RTMPCoreParameters.FLAG_DIRECTION_FLIP_VERTICAL) != 0) {
