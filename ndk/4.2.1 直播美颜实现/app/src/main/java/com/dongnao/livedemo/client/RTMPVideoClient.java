...
import com.dongnao.livedemo.core.CameraHelper;
import com.dongnao.livedemo.core.RESHardVideoCore;
import com.dongnao.livedemo.core.RESSoftVideoCore;
import com.dongnao.livedemo.core.RESVideoCore;
...
public class RTMPVideoClient {
    ...
    public RTMPVideoClient(RTMPCoreParameters parameters) {
    ...
    public boolean prepare(RTMPConfig resConfig) {
        synchronized (syncOp) {
            ...
            switch (resCoreParameters.filterMode) {
                case RTMPCoreParameters.FILTER_MODE_SOFT:
                    videoCore = new RESSoftVideoCore(resCoreParameters);
                    break;
                case RTMPCoreParameters.FILTER_MODE_HARD:
    ...
    public boolean startPreview(SurfaceTexture surfaceTexture, int visualWidth, int visualHeight) {
        synchronized (syncOp) {
            if (!isStreaming && !isPreviewing) {
                if (!startVideo()) {
                    resCoreParameters.dump();
                    LogTools.e("RTMPVideoClient,start(),failed");
                    return false;
                }
        ...
        synchronized (syncOp) {
            ...
            if (needRestartCamera) {
                ...
                if ((isPreviewing || isStreaming)) {
                    LogTools.d("RTMPClient,reSetVideoSize.restartCamera");
                    camera.stopPreview();
                    camera.release();
                    camera = null;
                    if (null == (camera = createCamera(currentCameraIndex))) {
    ...
    public BaseHardVideoFilter acquireHardVideoFilter() {
        if (resCoreParameters.filterMode == RTMPCoreParameters.FILTER_MODE_HARD) {
